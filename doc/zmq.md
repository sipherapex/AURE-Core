...
The Aure Core daemon can be configured to act as a trusted "border
router", implementing the aure wire protocol and relay, making
...
The ZeroMQ feature in Aure Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building aured:
...
For instance:
    ...
    $ aured -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubhashtx=tcp://192.168.1.2:28332 \
               -zmqpubrawtx=ipc:///tmp/aured.tx.raw \
...
These options can also be provided in aure.conf.
...
From the perspective of aured, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into aured directly. Furthermore, no information is
...
using. Aured appends an up-counting sequence number to each
