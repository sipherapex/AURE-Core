    ...
    const bool sign)
{
    ...
    while (true) {
        ...
        // the recipients are funded through the pegout kernel instead of traditional AURE outputs.
        ...
        // Calculate transaction size and the total necessary fee amount (includes AURE and MWEB fees)
    ...
    if (new_tx.mweb_type != MWEB::TxType::AURE_TO_AURE) {
...
bool TxAssembler::AttemptCoinSelection(InProcessTx& new_tx, const CAmount& nTargetValue) const
{
    ...
    if (new_tx.recipients.front().IsMWEB()) {
        ...
        // First try to construct a AURE-to-AURE transaction
        CoinSelectionParams mweb_to_mweb = new_tx.coin_selection_params;
        mweb_to_mweb.input_preference = InputPreference::AURE_ONLY;
        ...
        // If AURE-to-AURE fails, create a peg-out transaction
